...
def build():
    ...
    add_callout(doc, "新版向导怎么读", "这里现在只保留一个大框。点击“点击上传项目”，选择项目文件夹后，框内会按目录形式列出文件；确认文件夹选对后，再点击蓝色“识别项目”。")
    add_step(doc, "第一步", "点击“点击上传项目”", "选择要测试的项目文件夹。系统只读取文件夹结构来准备识别，不会修改项目代码；如果选错，可以点击“重新选择”。")
    add_step(doc, "第二步", "查看目录内容", "选择完成后，大框会显示项目名称、文件数量和目录内容。先确认里面能看到 package.json、src、app 或其他项目文件，再继续。")
    add_step(doc, "第三步", "点击“识别项目”", "系统会根据已选择的项目目录识别技术栈、启动方式、端口和访问地址。识别成功后，再按页面提示使用推荐设置并检查能否运行。")
    add_callout(doc, "小白判断标准", "你只需要完成“选择文件夹 → 确认目录 → 识别项目”。后续的启动命令、端口和健康检查由系统准备；完整技术信息仍可在高级区域查看。")
