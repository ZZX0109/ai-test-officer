...
function page(response, variant = "") {
  ...
  <section><h2>Approval</h2><input id="orderId" aria-label="Order ID" placeholder="ORD-1001"/><button id="approve">Approve order</button><p class="notice" data-testid="approval-status">pending</p></section>
  ...
  <script>
    ...
    document.querySelector('#apply').onclick=()=>{const role=document.querySelector('#role').value;document.querySelector('[data-testid=permission-status]').textContent=role==='viewer'&&variant!=='fxv_5e1f8b26c4a907d3'?'viewer: read-only':'reviewer: approve pending orders';};load();
