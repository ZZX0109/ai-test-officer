...
function page(response) {
  ...
  <script>
  const rows=document.querySelector('#rows');const status=document.querySelector('#status');const keyword=document.querySelector('#keyword');
  async function load(){const q=new URLSearchParams();if(status.value!=='all')q.set('status',status.value);if(keyword.value.trim())q.set('keyword',keyword.value.trim());const r=await fetch('/api/tasks?'+q);const data=await r.json();rows.innerHTML=data.tasks.map(t=>'<tr><td>'+t.title+'</td><td>'+t.status+'</td><td>'+t.owner+'</td></tr>').join('')||'<tr><td colspan="3">No tasks</td></tr>';}
