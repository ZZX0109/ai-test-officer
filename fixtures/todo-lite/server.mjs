...
function page(response, fault = "") {
  ...
  <script>
  const fault=${JSON.stringify(fault)};const rows=document.querySelector('#rows');const status=document.querySelector('#status');const keyword=document.querySelector('#keyword');
  async function load(){const q=new URLSearchParams();if(status.value!=='all')q.set('status',status.value);if(keyword.value.trim())q.set('keyword',keyword.value.trim());if(fault)q.set('faultProfile',fault);const r=await fetch('/api/tasks?'+q);const data=await r.json();if(!r.ok){rows.innerHTML='<tr><td colspan="3">API unavailable</td></tr>';return;}rows.innerHTML=data.tasks.map(t=>'<tr><td>'+t.title+'</td><td>'+t.status+'</td><td>'+t.owner+'</td></tr>').join('')||'<tr><td colspan="3">No tasks</td></tr>';}
  ...
  document.querySelector('#apply-role').onclick=()=>{const role=document.querySelector('#role').value;document.querySelector('[data-testid=permission-status]').textContent=role==='viewer'?'viewer: read-only':'editor: create and update';};load();
...
const server = http.createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/tasks" && request.method === "GET") {
    if (url.searchParams.get("faultProfile") === "api-503") return send(response, 503, { error: "fixture outage" });
